...
`--workspace <name|path>` and `--json <path>`. For `collect`, the system SHALL accept
only `--workspace <name|path>`, `--out <path>`, and `--json <path>`. The system SHALL
reject every other flag, including `--cwd`, `--root`, `--project`, `--allowed`,
`--allowed-packages`, include/exclude filters, depth/production/development flags,
markdown/tree output flags, and a workspace-all flag, with exit code 2.
...
#### Scenario: --cwd is not accepted
- **WHEN** the user runs `license-gate check --cwd /tmp/proj`
- **THEN** the CLI exits with code 2 because `--cwd` is unknown
...
### Requirement: Project root is process.cwd

The system SHALL treat `process.cwd()` as the project root. The system SHALL require
`package.json` to exist directly in `process.cwd()`. The system SHALL NOT walk upward
from `process.cwd()` looking for an ancestor `package.json` and SHALL NOT auto-detect a
parent project root. If `package.json` is missing in `process.cwd()`, the system SHALL
exit with code 2. If `package.json` declares an npm `workspaces` field, the project root
is treated as a workspace project root; otherwise it is treated as a single-package
project.

#### Scenario: no package.json in cwd
- **WHEN** the user runs `license-gate check` from a directory whose `package.json` does not exist
- **THEN** the CLI exits with code 2 with a message stating that `package.json` was not found in the current directory
...
- **WHEN** the user runs `license-gate check` from a directory whose `package.json` declares `workspaces`
...
- **WHEN** the user runs `license-gate check` from a directory whose `package.json` does not declare `workspaces`
...
The system SHALL use `@npmcli/arborist` with `new Arborist({ path: process.cwd() })`
...
The system SHALL always invoke Arborist with `path = process.cwd()` and SHALL NOT walk
upward to rescue a wrong working directory. If `process.cwd()` is itself a workspace
subdirectory, the system SHALL treat that directory as the project root and invoke
Arborist there. When `--workspace <name|path>` is used from a workspace project root,
the system SHALL NOT invoke Arborist with the selected workspace's path; instead,
Arborist still loads from `process.cwd()` and the workspace narrowing is performed inside
that already-loaded tree (see the Workspace narrowing requirement).
...
When `--workspace <name|path>` is provided, the system SHALL load Arborist from
`process.cwd()` (per the Graph discovery requirement) and SHALL NOT re-invoke or
re-point Arborist at the selected workspace's path. The system SHALL resolve the
workspace node inside the already-loaded tree by name (matching `tree.workspaces`) or by
path (relative paths resolved against `process.cwd()`, then matched against
`node.realpath`/`node.path`), and narrow the evaluation set to the workspace node plus
every node reachable from it via `node.edgesOut → edge.to`, deduplicated by `realpath`.
The system SHALL NOT filter by realpath prefix alone, because hoisted dependencies live
outside the workspace's realpath.
...
- **THEN** the system resolves the path against `process.cwd()` to a workspace node and narrows evaluation accordingly
...
`licenses/allowed-packages.txt`, resolved relative to `process.cwd()`. The system SHALL
NOT accept CLI flags, environment variables, or `package.json` config to relocate these
files. For `check`, the `licenses/allowed-hard.txt` file SHALL be required: when missing,
the system SHALL exit with code 2; the `licenses/allowed-packages.txt` file SHALL be
optional. The `collect` command SHALL NOT read either file.
...
The system SHALL implement `license-gate check` to: load the installed graph from
`process.cwd()`, narrow it if `--workspace` is supplied, parse
`licenses/allowed-hard.txt` and `licenses/allowed-packages.txt` (validating override
rules), evaluate every package per the literal-first algorithm, print a human-readable
policy result to stdout, optionally write JSON to `--json <path>`, and exit 0 (clean) /
1 (violations) / 2 (config or runtime error). The `check` command SHALL NOT accept
`--out`. The JSON file SHALL be awaited and closed before the process exits, including
when exiting with code 1.
...
The system SHALL implement `license-gate collect` to: load the installed graph from
`process.cwd()`, narrow it if `--workspace` is supplied, build a flat list of installed
package records (name, version, packageId, path, workspace, license, optional metadata
directly present in `package.json`), and emit a human-readable report. The system SHALL
NOT read `licenses/allowed-hard.txt` or `licenses/allowed-packages.txt` for `collect`,
and SHALL NOT make any policy decisions in `collect`. By default the report SHALL go to
stdout. With `--out <path>`, the report SHALL be written to the file and stdout SHALL
contain only a one-line "wrote N records to <path>" summary. With `--json <path>`,
...
commands or flags, `check --out`, missing `package.json` in `process.cwd()`, missing
